...
// ── Lesson 21 tests (carried forward) ───────────────────────────────────────

describe('21 — Result Combinators and Unwrap (carried forward)', () => {
  describe('fold()', () => {
...
describe('22 — Result.all and Advanced Inference', () => {
  // ── all() — runtime behavior ──────────────────────────────────────────────
  describe('all() — success cases', () => {
    it('returns Ok with all values when all inputs succeed', () => {
    ...
    it('has type Result<never, [number, string, boolean]> for all-ok inputs', () => {
      const result = all([ok(1), ok('two'), ok(true)]);
      if (result._tag === 'Ok') expectTypeOf(result.value).toEqualTypeOf<[number, string, boolean]>();
    });
    ...
    it('has type Result<never, []> for an empty array', () => {
      const result = all([]);
      if (result._tag === 'Ok') expectTypeOf(result.value).toEqualTypeOf<[]>();
    });
  });

  describe('all() — failure cases', () => {
    it('returns the first Err when any input fails', () => {
      const result = all([ok(1) as Result<string, number>, err('boom') as Result<string, number>, ok(3) as Result<string, number>]);
      expect(isErr(result)).toBe(true);
      if (isErr(result)) {
  ...
  // ── Type-level helpers ────────────────────────────────────────────────────
  describe('ResultValues<T>', () => {
    it('extracts a tuple of value types', () => {
      type Input = [Result<never, number>, Result<never, string>];
      expectTypeOf<ResultValues<Input>>().toEqualTypeOf<[number, string]>();
    });
  });

  describe('ResultError<T>', () => {
    it('extracts the union of error types', () => {
      type Input = [Result<string, number>, Result<Error, string>];
      expectTypeOf<ResultError<Input>>().toEqualTypeOf<string | Error>();
    });
  });
});
